...
const CURRENT_SLUG = "east-auckland";

/** Left rail: locations nav (current highlighted) + a contact CTA card. */
    ...
    <aside className="space-y-8">
      <nav aria-label="Locations">
        <ul className="overflow-hidden rounded-lg border border-border bg-white shadow-card">
          ...
          {locations.map((area) => {
            const active = area.slug === CURRENT_SLUG;
            return (
              <li
                key={area.slug}
      ...
      <div className="rounded-lg bg-surface p-6 text-center shadow-card">
        ...
        <p className="mt-2 text-sm text-muted">
          Tell us about your home and our East Auckland team will be in touch.
        </p>
...
/**
 * East Auckland location page body — two-column (sidebar + main), mirroring the
 * service-detail template. Static Server Component. Reviews, brands and the
 * closing CTA are composed by the route after this component.
 */
export function EastAuckland() {
  const {
    hero,
    keyTakeaways,
    whyChoose,
      ...
      <div className="grid gap-10 lg:grid-cols-[300px_1fr] lg:gap-12">
        ...
        <div className="order-1 space-y-14 lg:order-2">
          ...
          <ServiceAppointmentForm
            ...
            intro="Send us a few details and our East Auckland team will be in touch."
            note=""
          />
        </div>
      </div>
